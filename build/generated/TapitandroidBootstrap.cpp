...
#define TAG "com.tapit"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tapitandroid_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tapitandroid.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TapitandroidBindings::lookupGeneratedInit(
...
static void Tapitandroid_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tapitandroid_getBinding)->GetFunction());
}

static void Tapitandroid_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TapitandroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TapitandroidBinding = {
	"com.tapit",
	Tapitandroid_init,
	Tapitandroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_tapit_TapitandroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.tapit", &TapitandroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::TapitandroidBindings::lookupGeneratedInit));
}
